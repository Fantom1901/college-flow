...
class RoleChecker:
  ...
  async def __call__(
    ...
    # Безопасно извлекаем значение роли, приводим к нижнему регистру
    user_role_val = user.role.value if hasattr(user.role, 'value') else user.role
    user_role_normalized = str(user_role_val).lower()

    # Нормализуем разрешенные роли
    allowed_roles_normalized = [
      str(r.value if hasattr(r, 'value') else r).lower()
      for r in self.allowed_roles
    ]

    if user_role_normalized not in allowed_roles_normalized:
      logger.error(
        f"ACCESS DENIED: User {user.username} has role '{user_role_normalized}', allowed: {allowed_roles_normalized}")
      raise HTTPException(
        status_code=403,
